...
/client/verb/ooc(msg as text)
	...
	if(!mob)
		return
	if(IsGuestKey(key))
		to_chat(src, "<span class='danger'>Convidados não podem usar o OOC.</span>")
		return
	...
	if(!(prefs.toggles & CHAT_OOC))
		to_chat(src, "<span class='danger'>Você foi mutado do OOC.</span>")
		return

	if(!check_rights(R_ADMIN|R_MOD,0))
		if(!config.ooc_allowed)
			to_chat(src, "<span class='danger'>OOC foi globalmente mutado.</span>")
			return
		if(!config.dooc_allowed && (mob.stat == DEAD))
			to_chat(usr, "<span class='danger'>OOC para Mobs Mortos foi desligado.</span>")
			return
		if(prefs.muted & MUTE_OOC)
			to_chat(src, "<span class='danger'>Você não pode usar o OOC (mutado).</span>")
			return
...
/proc/toggle_ooc()
	...
	if (config.ooc_allowed)
		to_chat(world, "<B>O Canal de OOC foi globalmente ativado!</B>")
	else
		to_chat(world, "<B>O Canal de OOC foi globalmente desativado!</B>")
...
/client/proc/set_ooc(newColor as color)
	set name = "Definir cor do OOC de players"
	set desc = "Modifica a cor padrão do OOC dos Players."
	set category = "Server"
...
/client/proc/reset_ooc()
	set name = "Redefinir cor do OOC de players"
	set desc = "Retorna a cor padrão do OOC dos Players para o padrão."
	set category = "Server"

	if(!check_rights(R_SERVER))	return

	normal_ooc_colour = initial(normal_ooc_colour)
	message_admins("[key_name_admin(usr)] redefiniu a cor padrão do OOC dos Players")
	log_admin("[key_name(usr)] redefiniu a cor padrão do OOC dos Players")

	feedback_add_details("admin_verb","ROOC")

/client/proc/colorooc()
	set name = "Definir sua cor no OOC"
	set desc = "Faz com que você costumize sua cor no OOC."
	set category = "Preferences"

	if(!check_rights(R_ADMIN)) return

	var/new_ooccolor = input(src, "Por favor, selecione sua cor do OOC.", "Cor do OOC", prefs.ooccolor) as color|null
	if(new_ooccolor)
...
/client/proc/resetcolorooc()
	set name = "Redefinir sua cor no OOC"
	set desc = "Retorna sua cor do OOC para o padrão."
	set category = "Preferences"
...
/client/verb/looc(msg as text)
	...
	if(!(prefs.toggles & CHAT_LOOC))
		to_chat(src, "<span class='danger'>Você foi mutado do LOOC.</span>")
		return

	if(!check_rights(R_ADMIN|R_MOD,0))
		if(!config.looc_allowed)
			to_chat(src, "<span class='danger'>LOOC foi mutado globalmente.</span>")
			return
		if(!config.dooc_allowed && (mob.stat == DEAD))
			to_chat(usr, "<span class='danger'>LOOC foi mutado para Mobs Mortos.</span>")
			return
		if(prefs.muted & MUTE_OOC)
			to_chat(src, "<span class='danger'>Você não pode usar o LOOC(mutado).</span>")
			return
		if(handle_spam_prevention(msg,MUTE_OOC))
			return
		if(findtext(msg, "byond://"))
