...
/datum/controller/occupations/proc/JobLimitConditions(rank)
	if(rank in nonhuman_positions)
		return 1
	if(rank in list("Oficial da Marinha Espacial", "Oficial de operações especiais"))
		return 1

	var/list/step_1 = heads_positions + security_positions + supply_positions
	var/list/step_2 = engineering_positions + medical_positions + science_positions + list("Blueshield", "Representante da Federação")
