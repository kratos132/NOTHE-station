/datum/controller/gameticker/proc/scoreboard()
	...
	log_game("Os antagonistas foram..")
	...
	to_chat(world, "<b>A pontuação final da Crew foi:</b>")
...
/mob/proc/scorestats()
	var/dat = "<b>Estatisticas de pontuação do round</b><br><hr>"
	if(ticker && ticker.mode)
		dat += ticker.mode.get_scoreboard_stats()

	dat += {"
	<b><u>Estatisticas gerais.</u></b><br>
	<u>O bom:</u><br>

	<b>Itens uteis enviados:</b> [score_stuffshipped] ([score_stuffshipped * 5] Points)<br>
	<b>Colheitas hidroponicas:</b> [score_stuffharvested] ([score_stuffharvested * 5] Points)<br>
	<b>Miérios mineirados:</b> [score_oremined] ([score_oremined * 2] Points)<br>
	<b>Refrescos preparados:</b> [score_meals] ([score_meals * 5] Points)<br>
	<b>Pesquisas completas:</b> [score_researchdone] ([score_researchdone * 30] Points)<br>"}
	if (shuttle_master.emergency.mode == SHUTTLE_ENDGAME) dat += "<b>Escapada de Shuttles:</b> [score_escapees] ([score_escapees * 25] Points)<br>"
	dat += {"<b>Eventos Aleatorios sofridos:</b> [score_eventsendured] ([score_eventsendured * 50] Points)<br>
	<b>Estação inteiramente carregada:</b> [score_powerbonus ? "Yes" : "No"] ([score_powerbonus * 2500] Points)<br>
	<b>Estação Super limpa:</b> [score_mess ? "No" : "Yes"] ([score_messbonus * 3000] Points)<br><br>
	<U>O ruim:</U><br>

	<b>Corpos mortos na estação:</b> [score_deadcrew] (-[score_deadcrew * 25] Points)<br>
	<b>Messas não limpas:</b> [score_mess] (-[score_mess] Points)<br>
	<b>Problemas de energia na estação:</b> [score_powerloss] (-[score_powerloss * 20] Points)<br>
	<b>Doenças desenfreadas:</b> [score_disease] (-[score_disease * 30] Points)<br>
	<b>AI Destruida:</b> [score_deadaipenalty ? "Yes" : "No"] (-[score_deadaipenalty * 250] Points)<br><br>
	<U>O estranho</U><br>

	<b>Comida ingerida:</b> [score_foodeaten] bites/sips<br>
	<b>Vezes que o Clow foi abusado:</b> [score_clownabuse]<br><br>
	"}
	if (score_escapees)
		dat += {"<b>Foi embora rico:</b> [score_richestname], [score_richestjob]: $[num2text(score_richestcash,50)] ([score_richestkey])<br>
		<b>Foi embora maltratado:</b> [score_dmgestname], [score_dmgestjob]: [score_dmgestdamage] damage ([score_dmgestkey])<br>"}
	else
		if(shuttle_master.emergency.mode <= SHUTTLE_STRANDED)
			dat += "A estação não foi evacuada!<br>"
		else
			dat += "Ninguém escapou!<br>"

	dat += ticker.mode.declare_job_completion()

	dat += {"
	<hr><br>
	<b><u>PONTUAÇÃO FINAL: [score_crewscore]</u></b><br>
	"}

	var/score_rating = "Os aristocrátas!"
