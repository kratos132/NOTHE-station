...
/datum/objective/assassinate/find_target()
	...
	if(target && target.current)
		explanation_text = "Assasine [target.current.real_name], o [target.assigned_role]."
	else
...
/datum/objective/mutiny/rp/find_target()
	..()
	if(target && target.current)
		explanation_text = "Assassine, capiture ou converta [target.current.real_name], o [target.assigned_role]."
	else
		explanation_text = "Free Objective"
...
/datum/objective/maroon/find_target()
	..()
	if(target && target.current)
		explanation_text = "Previna [target.current.real_name], o [target.assigned_role] de escapar vivo."
	else
		explanation_text = "Free Objective"
	return target
...
/datum/objective/debrain/find_target()
	..()
	if(target && target.current)
		explanation_text = "Roube o cérebro [target.current.real_name] o [target.assigned_role]."
	else
		explanation_text = "Free Objective"
	return target
...
/datum/objective/hijack
	...
	explanation_text = "Sequestre a Shuttle sem nenhum membro Leal a FG dentro. \
	Agentes do sindicato, outros inimigos da FG , cyborgs, e pets estão apitos a embarcar vivos."
...
/datum/objective/hijackclone
	explanation_text = "Sequestre o shuttle de emergência garantindo que apenas você (ou suas cópias) vão escapar."
	martyr_compatible = 0
...
/datum/objective/block
	explanation_text = "Não permita que nenhuma forma de vida, seja orgânica ou sintética, escape no shuttle. AIs, Cyborgs e pAIs não são considerados vivos."
	martyr_compatible = 1
...
/datum/objective/escape
	explanation_text = "Escape no shuttle ou em uma Pod Espacial vivo."
...
/datum/objective/escape/escape_with_identity/find_target()
	...
	if(target && target.current)
		...
		explanation_text = "Escape na Shuttle ou numa Space Pod com a indentidade de [target_real_name], o [target.assigned_role] e usando seu ID Card."
	else
		explanation_text = "Free Objective"
...
/datum/objective/die
	explanation_text = "Morra uma morte gloriosa."
...
/datum/objective/destroy/find_target()
	...
	if(target && target.current)
		target_real_name = target.current.real_name
		explanation_text = "Destrua [target_real_name], a AI."
	else
		explanation_text = "Free Objective"
	return target
...
/datum/objective/blood/proc/gen_amount_goal(low = 150, high = 400)
	...
	explanation_text = "Acumule o total de [target_amount] unidades de sangue."
	return target_amount
...
/datum/objective/minimize_casualties
	explanation_text = "Minimise casualidades."
...
/datum/objective/heist/kidnap/choose_target()
	var/list/roles = list("Chefe de Engenharia","Diretor de Pesquisas","Roboticista","Químico","Engenheiro da Estação")
	var/list/possible_targets = list()
