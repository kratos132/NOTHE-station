...
/client/proc/response_team()
	set name = "Enviar Equipe de Atendimento da CentComm"
	set category = "Event"
	set desc = "Envia uma Equipe de Atendimento da CentComm para a Estação."

	if(!check_rights(R_EVENT))
		return

	if(!ticker)
		to_chat(usr, "\red O Round ainda não começou!")
		return
	...
	if(send_emergency_team)
		to_chat(usr, "\red A Central de Comando enviou uma Equipe de Atendimento a Emergências!")
		return

	if(alert("Você quer enviar uma ERT para a Estação?",,"Sim","Não") != "Sim")
		return
	...
	if(send_emergency_team)
		to_chat(usr, "\red A Central de comando já enviou uma Equipe de Atendimento a Emergências!")
		return
	...
	message_admins("[key_name_admin(usr)] está enviando uma Equipe de Atendimento a Emergências.", 1)
	log_admin("[key_name(usr)] eviou uma Equipe de Atendimento..")
	trigger_armed_response_team(ert_type)

/client/proc/pick_ert_type()
	switch(alert("Por favor selecione o tipo de ERT que deseja.", "Equipe de Atendimento a Emergências", "Codigo Amber", "Codigo Vermelho", "Codigo Gamma", "Cancelar"))
		if("Code Amber")
			if(alert("Confirmar: Enviar uma <b>ERT Leve</b> do Codigo Ambar?", "Equipe de Atendimento a Emergências", "Confirmar", "Cancelar") == "Confirmar")
				return new /datum/response_team/amber
			else
				return pick_ert_type()
		if("Code Red")
			if(alert("Confirmar: Enviar uma <b>ERT Média</b> do Codigo Vermelho?", "Equipe de Atendimento a Emergências", "Confirmar", "Cancelar") == "Confirmar")
				return new /datum/response_team/red
			else
				return pick_ert_type()
		if("Code Gamma")
			if(alert("Confirmar: Enviar uma <b>ERT de Elite</b> do Codigo Gamma?", "Equipe de Atendimento a Emergências", "Confirmar", "Cancelar") == "Confirmar")
