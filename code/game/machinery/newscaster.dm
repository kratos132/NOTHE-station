...
/obj/machinery/newscaster
	name = "newscaster"
	desc = "A standard Galactic Federation-licensed newsfeed handler for use in commercial space stations. All the news you absolutely have no use for, in one place!"
	...
	if(istype(user, /mob/living/carbon/human) || istype(user,/mob/living/silicon) )
		...
		switch(screen)
			if(0)
				...
				<BR><FONT SIZE=1>Propriedade da Federação</FONT>"}
				...
				if(src.securityCaster)
					...
					<BR><A href='?src=[UID()];menu_censor_channel=1'>Mark Feed Channel with FG D-Notice</A>
			...
			if(9)
				...
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with a FG D-Notice.<BR>"
			...
			if(10)
				dat+="<B>Galactic Federation Feed Censorship Tool</B><BR>"
				...
				dat+="<BR><A href='?src=[UID()];setScreen=[0]'>Cancel</A>"
			if(11)
				dat+="<B>FG D-Notice Handler</B><HR>"
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if(href_list["set_channel_name"])
			...
			if(choice=="Confirm")
				if(input_param==1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one. See the else below
					...
					if(news_network.wanted_issue.is_admin_message)
						alert("The wanted issue has been distributed by a FG higherup. You cannot edit it.","Ok")
						return
		...
		if(news_network.wanted_issue.is_admin_message)
			alert("The wanted issue has been distributed by a FG higherup. You cannot take it down.","Ok")
			return
		...
		if(FC.is_admin_channel)
			alert("This channel was created by a FG officer. You cannot censor it.","Ok")
			return
		...
		if(MSG.is_admin_message)
			alert("This message was created by a FG Officer. You cannot censor its author.","Ok")
			return
		...
		if(FC.is_admin_channel)
			alert("This channel was created by a FG Officer. You cannot place a D-Notice upon it.","Ok")
			return
...
/obj/item/weapon/newspaper
	name = "newspaper"
	desc = "An issue of The Griffon, the newspaper circulating aboard Federation Space Stations."
	...
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				...
				dat+="<DIV ALIGN='center'><FONT SIZE=2>Nanotrasen-standard newspaper, for use on Federation Space Facilities</FONT></div><HR>"
